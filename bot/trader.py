...
class Trader:
    ...
    def stash_items(self):
        ...
        for loc in item_locations:
            self.input_handler.inventory_click(
                loc[0], loc[1], inventory_base=OWN_INVENTORY_ORIGIN, ctrl_click=True
            )
        ...
        while not trade_partner_in_area:
            while self.trade_queue.empty():
                ...
                for message in messages:
                    if (
                        message["text"] == self.invite_message
                        and message["type"] == "whisper"
                    ):
    ...
    def tail(self, set_seeker=False, max_age=None):
        ...
        if max_age is None:
            ...
            messages = [
                self._split_chat_msg(line)
                for line in lines
                if self._split_chat_msg(line)["time"] > datetime.now() - max_age
            ]
        if messages:
    ...
    def _get_partners_trade_items(self):
        ...
        while datetime.now() < timeout:
            ...
            self.input_handler.inventory_click(
                -1, -1, inventory_base=TRADE_PARTNER_ORIGIN
            )
    ...
    def _close_colors(self, target_color, actual_color, max_distance=16):
        ...
        distance = math.sqrt(
            (actual_color[0] - target_color[0]) ** 2
    ...
    def _find_stash(self):
        ...
        if len(stash_corner[0]) > 0:
            return [
                stash_corner[1][0] / self.resolution[0],
